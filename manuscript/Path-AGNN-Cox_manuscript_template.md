...
Path-AGNN-Cox couples a pathway-constrained GNN with patient-specific attention to a formal statistical framework for testing and clinically anchoring patient-specific pathway rewiring, while offering discrimination comparable to deep survival baselines. With {{N_DATASETS}} internal and {{N_EXTERNAL}} external cohorts, open-source code at https://github.com/ZhongdaHospital-SEU/Path-AGNN-Cox, a versioned Zenodo archive at https://doi.org/10.5281/zenodo.22030045, and a pip-installable Python package, the framework is directly reusable for pan-cancer prognostic modeling and for interrogating the patient-specific graph dynamics that static pathway models cannot produce.
