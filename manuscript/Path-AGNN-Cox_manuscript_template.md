...
Expression matrices were first mapped to the KEGG cancer-core pathway catalogue [19] ({{N_PATHWAYS}} pathways, {{GENES_UNION}} genes in the union; compiled from the KEGG_2021_Human.gmt gene-set collection, KEGG release 2021). Pathway modules containing fewer than three or more than 300 genes were excluded from the model input. After intersection with each cohort's measured genes, on average {{GENES_AVG}} genes per cohort were retained; genes outside any pathway were excluded, so that every model in the benchmark operates on the same pathway-mapped gene universe (a matched comparison).
