...
## 方案 B 补充：GSE91061 On 时间点敏感性分析（2026-08-22）

### 数据与处理
- 58 个 On 治疗中活检样本（57 例患者，Pt109 双活检）；GEO 系列矩阵中 On 样本无 response 字段，响应为患者级最佳总体缓解，从同系列 Pre 行按患者映射；42 例有响应注释（9 响应/33 非响应）。
- 模板与 Pre 分析一致（SKCM，TCGA OS 事件分层相关差；队列内 z-score）；配对分析在 Pre+On 合并池内 z-score，组间差值设计可抵消时间点共有批次位移。
- 患者级输出 `gse91061_on_expr.csv`、`gse91061_on_clinical.csv`、`gse91061_paired_delta.csv` 不入库。

### 结果
| 分析 | n | g | Wilcoxon P | 随机 null 百分位 |
|---|---|---|---|---|
| 横断面 On | 42 | -0.50 | 0.032 | 38.5%（2,850 随机基因集） |
| 配对 Pre→On Δ | 42 | -0.79 | 0.081 | 8.5% |
